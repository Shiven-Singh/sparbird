...
export function PersonaCard({ persona }: { persona: PersonaSpec }) {
  return (
    <Link
      ...
      className="panel group flex h-full flex-col p-4 transition-colors hover:bg-panel-2"
    >
      {/* the stage: a lit figure on a dark ground */}
      <div className="relative flex h-40 items-end justify-center overflow-hidden bg-[radial-gradient(ellipse_at_50%_100%,rgba(255,255,255,0.10),rgba(255,255,255,0)_70%)]">
        <Avatar seed={persona.id} className="h-[152px] w-[152px] translate-y-2" />
        <span className="label absolute top-3 right-3 bg-bg/70 px-1.5 py-1 text-text backdrop-blur">
